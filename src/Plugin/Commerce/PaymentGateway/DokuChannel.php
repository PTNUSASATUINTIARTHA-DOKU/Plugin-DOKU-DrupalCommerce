 ...
 *   id = "doku_channel",
 *   label = "Doku Channel",
 *   display_label = "Online Payment via Doku Channel",
 *    forms = {
 *     "offsite-payment" = "Drupal\commerce_doku\PluginForm\DokuChannelForm",
 *   },
*   modes= {
 *     "staging" = "Staging",
 ...
 * )
 */
class DokuChannel extends OffsitePaymentGatewayBase implements DokuChannelInterface {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['channel'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Payment Channel'),
      '#description' => $this->t('Input your Doku Payment Channel.'),
  ...
  public function submitConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!$form_state->getErrors()) {    
      ...
      $this->configuration['channel'] = $values['channel'];
    }
  }
  ...
  public function add_doku($datainsert) 
{
  ...
  $db_prefix = $databases['default']['default']['prefix'];
    ...
    protected function loadPaymentByOrderId($order_id) {
        ...
        $payment_by_order_id = $storage->loadByProperties(['remote_id' => 'dokuid']);
        if (empty($payment_by_order_id)) {
          return NULL;
        }
   ...
   * @return string
   */
  public function getPaymentRedirectBCA() {
    $config = $this->getConfiguration();
    $dokuredirect = 'https://pay.doku.com/Suite/ReceiveMIP';
    if ($config['mode'] == 'staging') {
      $dokuredirect = 'https://staging.doku.com/Suite/ReceiveMIP';
    }
    return $dokuredirect;
  }
   ...
   * {@inheritdoc}
   */
  public function onNotify($order_id){
  $payment = $this->loadPaymentByOrderId($order_id);
  $payment->setState('complate');
  $payment->save();
// echo 'test'.$payment;
  echo 'tailah'.$order_id.'fuck';
  }
   ...
   */
    ...
    $payment_channel = $configuration['channel'];
    ...
    if ($payment_channel=='37'){
      $params['SHIPPING_COUNTRY'] = $address->getCountryCode();
      $params['SHIPPING_ADDRESS'] = $address->getAddressLine1().' '.$address->getAddressLine2();
      $params['SHIPPING_CITY'] = $address->getLocality();
      $params['SHIPPING_ZIPCODE'] = $address->getPostalCode();
    }
